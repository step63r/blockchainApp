...
import { Alert, Avatar, Button, Card, Container, Group, SimpleGrid, Stack, Text, TextInput, Title, Badge } from "@mantine/core";
...
export default function MyGovernor() {
  ...
  const [myGovernorContract, setMyGovernorContract] = useState<types.MyGovernor | null>(null);
  const [myERC20Contract, setMyERC20Contract] = useState<types.MyERC20 | null>(null);

  // 自身のアドレスが委任済みかどうかの状態を保持するState
  const [isDelegated, setIsDelegated] = useState(false);

  // Alert, Loading State
  const [showAlert, setShowAlert] = useState(false);  // Alertの表示管理
  const [alertMessage, setAlertMessage] = useState(''); // Alert meesage
  ...
  // My GovernorとMyERC20のコントラクトのインスタンスをethers.jsを利用して生成
  useEffect(() => {
    if (signer) {
      const governorContract = types.MyGovernor__factory.connect(governorContractAddress, signer);
      setMyGovernorContract(governorContract);
      const erc20Contract = types.MyERC20__factory.connect(erc20ContractAddress, signer);
  ...
  return (
    <div>
      <Title order={1} style={{ paddingBottom: 12 }}>My Governor Management</Title>
      ...
      <SimpleGrid cols={{ base: 1, sm: 3, lg: 5 }}>
        <CreateProposalForm
          ...
          onCreateProposal={createProposal} loading={loading} />
      ...
      <Group justify="space-between" mt="md" mb="xs">
        ...
        {votesStatus && proposalStatus != ProposalStatus.Pending && (
          <Stack spacing="xs" direction="column">
            <Text size="sm">Against Votes: {votesStatus.againstVotes.toString()}</Text>
            <Text size="sm">For Votes: {votesStatus.forVotes.toString()}</Text>
            <Text size="sm">Abstain Votes: {votesStatus.abstainVotes.toString()}</Text>
          </Stack>
        )}
